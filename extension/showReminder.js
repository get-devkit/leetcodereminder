...
async function goToRandomQue() {
  return new Promise(async (resolve, reject) => {
    const randomQueTitle = await fetch(`${serverProxy}/randomEasyQue`).catch(
      () => {
        reject("Error Occured During Getting Random Que");
      }
    );
...
async function showReminder() {
  ...
  let text = encodeURIComponent(
    arr[(Math.floor(Math.random() * 10000) % 10) % arr.length]
  );
  ...
  cat = `https://cataas.com/${dum.url.split("?")[0]}/says/${text}?${
    dum.url.split("?")[1]
  }`;
  ...
  randomQueBtn.onclick = async () => {
    window.open(await goToRandomQue());
  };
}
